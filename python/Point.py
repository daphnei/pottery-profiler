...
class Point:
	...
	def is_between(self, other1, other2):
		'''Check if this point is somewhere in between the other two points.
		   In other words, return true if a line segmenet going between other1 and
		   other2 would also intersect with this point.
		'''

		colinear = (other2.x - other1.x) * (self.y - other1.y) == (self.x - other1.x) * (other2.y - other1.y)
		within = other1.x <= self.x <= other2.x or other1.x >= self.x >= other2.x if not other1.x == other2.x \
				 else other1.y <= self.y <= other2.y or other1.y >= self.y >= other2.y
